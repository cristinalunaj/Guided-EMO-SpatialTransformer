...
def fill_df(df_in_out, df_in):
    for i, row in df_in.iterrows():
        if (row["path"]) in list(df_in_out["path"].values):
            ...
            df2 = pd.DataFrame([[row["path"], 1, -2, -2, row["expression"]]],
                               columns=["path", "count", "valence", "arousal", "expression"])
            df_in_out = df_in_out.append(df2)
    return df_in_out


def analyse_metrics_fer2013(df_erros):
    ...
    return df_erros_PosNeg, df_erros_NegPos
...
if __name__ == '__main__':
    ...
    out_path = "/mnt/ESITUR2/DATASETS/FER2013/fer2013/ZZEVAL_ERROR_MODELS/more_problematics_all"
    ...
    for fold in range(0,5):
        ...
        df_PosPred_NegLabel = pd.DataFrame([], columns=["path",  "count", "emotion"])
        df_NegPred_PosLabel = pd.DataFrame([], columns=["path",  "count", "emotion"])
        for file in os.listdir(logs_path):
            ...
            df_preds = df_preds.drop(["idx"], axis=1)
            compl_df_pred = complete_df.loc[complete_df["path"].isin(list(df_preds["path"]))]

            compl_df_pred = compl_df_pred.sort_values(by="path")
            compl_df_pred = compl_df_pred.reset_index(drop=True)
            df_preds = df_preds.sort_values(by="path")
            df_preds = df_preds.reset_index(drop=True)


            df_preds["expression"] = compl_df_pred["emotion"]
            ...
            df_erros_PosNeg, df_erros_NegPos = analyse_metrics_fer2013(error_df)
            # Save imgs of errors
