...
def define_paths(current_path, out_path, args, name_imgs_path="images/"):
    ...
    images_path = results_path + name_imgs_path
...
def main():
    ...
    parser.add_argument("-o", "--out-path", default=default_data_path,
                        help="specify the path where saliency images will be \
                                 stored")
    parser.add_argument("-Ds", "--data_set", default="RAVDESS",
                        help="specify the Dataset to use: ['RAVDESS' or 'FER2013']")

    args = parser.parse_args()

    if(args.data_set=="AffectNet"):
        root_folder = (args.path).rsplit("/", 1)[0]

        for subfolder in os.listdir(root_folder):
            print("Processing folder: ", subfolder)
            complete_path = os.path.join(root_folder, subfolder)
            args.path = complete_path
            if(os.path.exists(os.path.join(args.out_path, "results", subfolder))):continue
            paths = define_paths(default_data_path, args.out_path, args, name_imgs_path=subfolder+"/")
            try:
                if args.phase == "train":
                ...
                print("No files in: ", complete_path)

    else:
        paths = define_paths(default_data_path, args.out_path, args)
